(function installPortableFluentIcons(){
  ...
  function svgFallback(name){
    const path = LOCAL_FALLBACK[name] || LOCAL_FALLBACK.Component || LOCAL_FALLBACK.Add;
    return '<svg class="fluent-svg-icon fluent-svg-fallback" viewBox="0 0 24 24" aria-hidden="true" focusable="false">' + path + '</svg>';
  }
  ...
  function sanitizeSvg(svgText){
    ...
    svg.querySelectorAll('*').forEach(node => {
      [...node.attributes].forEach(attr => {
        ...
        if (attrName.startsWith('on') || attrName === 'href' || attrName.endsWith(':href') || attrName === 'style') {
    ...
    svg.setAttribute('class', 'fluent-svg-icon fluent-svg-official');
